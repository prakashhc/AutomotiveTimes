...
function setComments($conn){
  if (isset($_POST['commentSubmitAccord'])){
    $uid = $_POST['uid'];
    ...
    $sql = "INSERT INTO commentsaccord (uid, date, message) VALUES ('$uid', '$date', '$message')";
...
function getComments($conn){
  $sql = "SELECT * FROM commentsaccord";
  $result = $conn->query($sql);
  while($row = $result->fetch_assoc()){
    ...
    echo "</p>
      <form class='delete-form' method='POST' action='".deleteComments($conn)."'>
        ...
        <button name='deleteComment' type='submit'>Delete</button>
      </form>
      <form class='edit-form' method='POST' action='editcommentaccord.php'>
        <input type='hidden' name='cid' value='".$row['cid']."'>
...
function upvote($conn) {
  if (isset($_POST['upvoteButton'])) {
    ...
    $sql = "SELECT upVotes FROM commentsaccord WHERE cid = ?";
    ...
    $updateSql = "UPDATE commentsaccord SET upVotes = ? WHERE cid = ?";
...
function downvote($conn) {
  if (isset($_POST['downvoteButton'])) {
    ...
    // Fetch the current 'upVotes' count for the comment with $cid
    $sql = "SELECT downVotes FROM commentsaccord WHERE cid = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $cid);
    $stmt->execute();
    ...
    // Update the 'upVotes' value in the database
    $updateSql = "UPDATE commentsaccord SET DownVotes = ? WHERE cid = ?";
    $updateStmt = $conn->prepare($updateSql);
...
function editComments($conn){
  if (isset($_POST['commentSubmitAccord'])){
    ...
    $newDates = date('Y-m-d H:i:s');
    $sql = "UPDATE commentsaccord SET message='$message', date='$newDates' WHERE cid='$cid'";
...
function deleteComments($conn){
  if (isset($_POST['deleteComment'])){
    $cid = $_POST['cid'];
    $sql = "DELETE FROM commentsaccord WHERE cid = '$cid'";
    $result = $conn->query($sql);
    header("Location: accord.php");
  }
}
